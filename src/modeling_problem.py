...
# Objective
def CalcSchedule(M):
    return sum(sum(sum(M.Ratings[t, d, p]*M.WalkNeeded[t, d]*M.Assignments[t, d, p] for p in M.People) for d in M.Dates) for t in M.Times) - M.AllWalkMultiplier*sum(sum(M.IsThreeWalks[d, p] for d in M.Dates) for p in M.People)
...
def EnsureDurationsSimilar(M, p):
    return sum(sum(M.Lengths[t]*M.Assignments[t, d, p] for d in M.Dates) for t in M.Times) <= M.MaxWalkPortion*sum(sum(M.Lengths[t]*M.WalkNeeded[t, d] for d in M.Dates) for t in M.Times)
