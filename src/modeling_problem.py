...
# Parameters
M.WalkNeeded = Param(M.Times, M.Dates, within=Binary)
M.Ratings = Param(M.Times, M.Dates, M.People, within=NonNegativeIntegers)
...
# Objective
def CalcSchedule(M):
    return sum(sum(sum(M.Ratings[t, d, p]*M.WalkNeeded[t, d]*M.Assignments[t, d, p] for p in M.People) for d in M.Dates) for t in M.Times) + M.ThreeWalkFactor*sum(sum(2 - sum(M.Assignments[t, d, p] for t in M.Times) for d in M.Dates) for p in M.People)
